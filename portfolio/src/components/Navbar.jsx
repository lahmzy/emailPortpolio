  ...
  return (
    <header className="flex shadow-lg  relative justify-between items-center p-6 bg-gradient-to-br w-full from-gray-400 to-primaryBg ">
      <div className="md:w-60 w-40 h-auto rounded-md px-2">
        <img src="/img/logo2.png" className="w-[3.5rem] h-[3.5rem] rounded-full" />
      </div>
      ...
      {menuIcon && (
        <div
          className={`absolute flex flex-col ${
            mobileMenu && "hidden"
          }  top-[7rem] rounded-md items-center py-8 mt-10 font-semibold gap-2 z-10  shadow-md right-6 bg-white left-6 drop-shadow-md`}
        >
          {links.map((item) => (
            <div
              onClick={() => {
